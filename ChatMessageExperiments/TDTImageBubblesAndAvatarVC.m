...
@interface TDTImageBubblesAndAvatarVC ()
...
@implementation TDTImageBubblesAndAvatarVC
...
-(void) willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    for (NSIndexPath *path in [self.tableView indexPathsForVisibleRows]) {
        ...
        for(TDTImageBubbleAndAvatarView *obj in tempCell.contentView.subviews)
        {
...
{
    static NSString *CellIdentifier = @"avatarAndBubbleImageCell";
    ...
    TDTImageBubbleAndAvatarView *view = [[TDTImageBubbleAndAvatarView alloc] initWithFrame:CGRectZero withType:data.type withSize:data.sizeOfBubble withBubbleImage:((data.type == sent)? self.bubbleImageSent:self.bubbleImageReceived) withAvatarImage:((data.type == sent)? self.avatar1:self.avatar2)];

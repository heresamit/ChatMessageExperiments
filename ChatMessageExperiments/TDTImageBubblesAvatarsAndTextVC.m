//
//  TDTImageBubblesAvatarsAndTextVC.m
...
@interface TDTImageBubblesAvatarsAndTextVC ()
...
@implementation TDTImageBubblesAvatarsAndTextVC
...
-(void) willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    for (NSIndexPath *path in [self.tableView indexPathsForVisibleRows]) {
        ...
        for(TDTImageBubbleAvatarAndTextView *obj in tempCell.contentView.subviews)
        {
...
{
    static NSString *CellIdentifier = @"ImageMethodCells";
    ...
    TDTImageBubbleAvatarAndTextView *view = [[TDTImageBubbleAvatarAndTextView alloc] initWithFrame:CGRectZero withType:data.type withSize:data.sizeOfBubble withBubbleImage:((data.type == sent)? self.bubbleImageSent:self.bubbleImageReceived) withAvatarImage:((data.type == sent)? self.avatar1:self.avatar2) withText:data.text];

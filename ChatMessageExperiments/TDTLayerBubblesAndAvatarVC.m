...
@interface TDTLayerBubblesAndAvatarVC ()
...
@implementation TDTLayerBubblesAndAvatarVC
...
-(void) willRotateToInterfaceOrientation:(UIInterfaceOrientation)toInterfaceOrientation duration:(NSTimeInterval)duration
{
    ...
    for (NSIndexPath *path in [self.tableView indexPathsForVisibleRows]) {
        ...
        for(TDTLayerBubbleAndAvatarView *obj in tempCell.contentView.subviews)
        {
...
{
    static NSString *CellIdentifier = @"avatarAndBubbleLayerCell";
    ...
    TDTLayerBubbleAndAvatarView *view = [[TDTLayerBubbleAndAvatarView alloc] initWithFrame:CGRectZero withType:data.type withSize:data.sizeOfBubble withBubbleImage:((data.type == sent)? self.bubbleImageSent:self.bubbleImageReceived) withAvatarImage:((data.type == sent)? self.avatar1:self.avatar2)];
